...
int my_input(In &in)
{
  ...
  in.units = 0;         // 0 for lj, 1 for metal
  in.datafile = NULL;
...
/*
  ...
  */
  ...
  MMD_float new_cutneighsq[ntypes * ntypes];
  neighbor.cutneighsq = new_cutneighsq;

  Integrate integrate;
  Thermo thermo;

  Comm comm;
  MMD_float new_buf_send[MAXBUF];
  MMD_float new_buf_recv[MAXBUF];
  comm.buf_send = new_buf_send;
  comm.buf_recv = new_buf_recv;
...
*/
    ...
    atom.natoms = 4 * in.nx * in.ny * in.nz;
    int total = atom.natoms * PAD * 5 + 1024 + 1;
    MMD_float new_x[total];
    MMD_float new_v[total];
    MMD_float new_f[total];
    MMD_float new_xold[total];
    MMD_float new_x_copy[total];
    MMD_float new_v_copy[total];
    int new_type[atom.natoms * 3];
    int new_type_copy[atom.natoms * 3];

    atom.x = new_x;
    atom.v = new_v;
    atom.f = new_f;
    atom.xold = new_xold;
    atom.x_copy = new_x_copy;
    atom.v_copy = new_v_copy;
    atom.type = new_type;
    atom.type_copy = new_type_copy;
    ...
    MMD_int new_steparr[maxstat];
    MMD_float new_tmparr[maxstat];
    MMD_float new_engarr[maxstat];
    MMD_float new_prsarr[maxstat];

    thermo.steparr = new_steparr;
    thermo.tmparr = new_tmparr;
    thermo.engarr = new_engarr;
    thermo.prsarr = new_prsarr;
...
*/
  ...
  // #pragma omp parallel
  {
    ...
    int new_numneigh[nmax];
    int new_neighbors[nmax * MAXNEIGHS];
    neighbor.numneigh = new_numneigh;
