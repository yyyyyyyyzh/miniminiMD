...
#define MAXNEIGHS 100                   // TODO: before run, check if maxneighs is big enough
#define ATOMS_PER_BIN 32                // TODO: before run, check if atoms_per_bin is big enough
#define MBINS 2000
#define STENCIL_NUM 50

int new_bins[MBINS * 1 * ATOMS_PER_BIN];    // mbins * num_omp_threads * atoms_per_bin
int new_bincount[MBINS * 1];                // mbins * num_omp_threads
int stencil[STENCIL_NUM];
...
Neighbor::Neighbor(int ntypes_)
{
  ...
  maxneighs = 100;
  nmax = 0;
  bincount = new_bincount;
  bins = new_bins;
  atoms_per_bin = 32;       // TODO: check if atoms_per_bin is big enough
  // stencil = nullptr;
  threads = nullptr;
  halfneigh = 0;
  ghost_newton = 1;
...
int Neighbor::setup(Atom &atom)
{
  ...
  */
  ...
  nmax = (2 * nextz + 1) * (2 * nexty + 1) * (2 * nextx + 1);     // TODO: get num of nmax and define in macro definition NMAX
  ...
  mbins = mbinx * mbiny * mbinz;      // TODO: before run, let MBINS == mbins
